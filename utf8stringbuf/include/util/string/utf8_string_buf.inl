...
/*#ifndef UTF8_STRING_BUF_INL_*/
/*#define UTF8_STRING_BUF_INL_*/
...
/*!
 *  \enum utf8_string_buf_bool_enum
 *  \private
 */
/*  enumeration for true and false or success and failure */
enum utf8_string_buf_bool_enum {UTF8_STRING_BUF_FALSE=0, UTF8_STRING_BUF_TRUE=1,};

/*!
 *  \enum utf8_string_buf_search_enum
 *  \private
 */
/*  enumeration for search pattern not found */
enum utf8_string_buf_search_enum {UTF8_STRING_BUF_NOT_FOUND=-1,};

/*!
 *  \var utf8_string_buf_private_format_signed_64_bit_int
 *  \private
 */
/*  a character array containing nothing but a single zero */
extern const char *utf8_string_buf_private_format_signed_64_bit_int;

/*!
 *  \var utf8_string_buf_private_format_64_bit_hex
 *  \private
 */
/*  a character array containing nothing but a single zero */
extern const char *utf8_string_buf_private_format_64_bit_hex;

/*!
 *  \var utf8_string_buf_private_empty_buf
 *  \private
 */
/*  a character array containing nothing but a single zero */
extern char utf8_string_buf_private_empty_buf[1];

/*!
 *  \fn utf8_string_buf_private_make_null_termination( utf8_string_buf_t this_ )
 *  \private
...
extern unsigned int utf8_string_buf_private_make_null_termination( utf8_string_buf_t this_ );

/*!
 *  \fn utf8_string_buf_private_replace_region_by_str( utf8_string_buf_t this_, unsigned int this_Length, int start, int length, const char *replacement )
 *  \private
 */
/* function to replace a region within a string buffer, the original string length must be provided in this_Length */
extern utf8_error_t utf8_string_buf_private_replace_region_by_str( utf8_string_buf_t this_, unsigned int this_Length, int start, int length, const char *replacement );

static inline utf8_string_buf_t utf8_string_buf( char *that ) {
    utf8_string_buf_t result;
    if ( that == NULL ) {
        result.size = 1;
        result.buf = utf8_string_buf_private_empty_buf;
    }
    else {
...
static inline int utf8_string_buf_find_first_buf( const utf8_string_buf_t this_, const utf8_string_buf_t pattern ) {
    int result = UTF8_STRING_BUF_NOT_FOUND;
...
static inline int utf8_string_buf_find_last_buf( const utf8_string_buf_t this_, const utf8_string_buf_t pattern ) {
    int result = UTF8_STRING_BUF_NOT_FOUND;
    int thisLen = utf8_string_buf_get_length( this_ );
...
static inline utf8_error_t utf8_string_buf_copy_str( utf8_string_buf_t this_, const char *original ) {
    ...
    if ( original == NULL ) {
        this_.buf[0] = '\0';
        complete = UTF8_ERROR_NULL_PARAM;
    }
    else {
...
static inline utf8_error_t utf8_string_buf_replace_all_str_by_str( const utf8_string_buf_t this_, const char *pattern, const char *replacement ) {
    ...
    if ( pattern != NULL ) {
        ...
        result = utf8_string_buf_replace_all( this_, patterns_and_replacements );
    }
    return result;
}
...
static inline utf8_error_t utf8_string_buf_replace_all_buf_by_buf( const utf8_string_buf_t this_, const utf8_string_buf_t pattern, const utf8_string_buf_t replacement ) {
    ...
    return utf8_string_buf_replace_all( this_, patterns_and_replacements );
}

static inline utf8_error_t utf8_string_buf_append_str( utf8_string_buf_t this_, const char *appendix ) {
    utf8_error_t result = UTF8_ERROR_SUCCESS;
    if ( appendix == NULL ) {
...
static inline utf8_error_t utf8_string_buf_append_int( utf8_string_buf_t this_, const int64_t appendix ) {
    ...
    sprintf( numberStr, utf8_string_buf_private_format_signed_64_bit_int, appendix );
    return utf8_string_buf_append_str( this_, numberStr );
}
...
static inline utf8_error_t utf8_string_buf_append_hex( utf8_string_buf_t this_, const uint64_t appendix ) {
    ...
    /* Note: snprintf is not available on every OS */
    sprintf( numberStr, utf8_string_buf_private_format_64_bit_hex, appendix );
    return utf8_string_buf_append_str( this_, numberStr );
}
...
static inline utf8_error_t utf8_string_buf_replace_region_by_str( utf8_string_buf_t this_, int start, int length, const char *replacement ) {
    unsigned int this_Length = utf8_string_buf_get_length( this_ );
...
static inline utf8_error_t utf8_string_buf_delete_from_end( utf8_string_buf_t this_, int length ) {
    int this_Length = utf8_string_buf_get_length( this_ );
