...
static inline bool json_writer_contains_xml_tag_name_characters ( json_writer_t *this_, utf8stringview_t string_view )
{
    ...
    while( utf8codepointiterator_has_next( &it ) && ( ! result ) )
    {
        ...
        if ( json_writer_private_is_xml_tag_name_character( this_, utf8codepoint_get_char( next ), true ) )
        {
            result = true;
        }
    }
...
static inline int json_writer_write_xml_tag_name_characters ( json_writer_t *this_, utf8stringview_t string_view )
{
    ...
    {
        utf8codepoint_t next = utf8codepointiterator_next( &it );
        if ( json_writer_private_is_xml_tag_name_character( this_, utf8codepoint_get_char( next ), is_start ) )
        {
            if ( is_start )
            {
                is_start = false;
...
static inline void json_writer_reset_indent ( json_writer_t *this_ )
{
    ...
    json_writer_private_update_encoding_tables( this_ );
}
