...
{
    /* sub test case void */
    const char * name_void = data_relationship_type_get_name( DATA_RELATIONSHIP_TYPE_VOID );
    TEST_EXPECT_EQUAL_STRING( "", name_void );

    /* sub test case valid */
    const char * name_valid = data_relationship_type_get_name( DATA_RELATIONSHIP_TYPE_UML_COMMUNICATION_PATH );
    TEST_EXPECT_EQUAL_INT( 18, utf8string_get_length(name_valid) );
