...
u8_error_t data_node_set_load( data_node_set_t *this_, data_row_id_t classifier_id, data_database_reader_t *db_reader )
{
    ...
    else
    {
        ...
        result |= data_database_reader_get_features_by_classifier_id( db_reader,
                                                                      classifier_id,
                                                                      &feature_iterator
                                                                    );
        for ( int_fast32_t f_idx = 0; (f_idx < DATA_NODE_SET_MAX_FEATURES) && data_feature_iterator_has_next( &feature_iterator ); f_idx ++ )
        {
        ...
        if ( data_feature_iterator_has_next( &feature_iterator ) )
        {
            ...
            U8_LOG_ERROR( "U8_ERROR_ARRAY_BUFFER_EXCEEDED at loading features of a classifier" );
        }
        ...
        result |= data_database_reader_get_relationships_by_classifier_id( db_reader,
                                                                           classifier_id,
                                                                           &rel_iterator
                                                                         );
        for ( int_fast32_t r_idx = 0; (r_idx < DATA_NODE_SET_MAX_RELATIONSHIPS) && data_relationship_iterator_has_next( &rel_iterator ); r_idx ++ )
        {
        ...
        {
            result |= U8_ERROR_ARRAY_BUFFER_EXCEEDED;
            U8_LOG_ERROR( "U8_ERROR_ARRAY_BUFFER_EXCEEDED at loading relationships of a classifier" );
        }
