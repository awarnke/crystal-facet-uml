...
void gui_resource_selector_init ( gui_resource_selector_t *this_, gui_resources_t *resources )
{
    ...
    (*this_).classifier_types_start = idx;
    {
        ...
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_CONSTRAINT_PROPERTY, "Constraint Property/Equation (parametric)", icon_clas_constraint );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_clas_node = gui_resources_get_type_clas_node( (*this_).resources );
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_UML_NODE, "Node (deployment)", icon_clas_node );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        ...
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_FEATURE, "Feature", icon_clas_requirement );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        ...
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_UML_ACTOR, "Actor (use case)", icon_clas_actor );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_clas_usecase = gui_resources_get_type_clas_usecase( (*this_).resources );
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_UML_USE_CASE, "Use Case (use case)", icon_clas_usecase );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_clas_boundary = gui_resources_get_type_clas_boundary( (*this_).resources );
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_UML_SYSTEM_BOUNDARY, "System Boundary (use case)", icon_clas_boundary );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_clas_diagram = gui_resources_get_type_clas_diagram( (*this_).resources );
        gui_type_resource_init_classifier( current, DATA_CLASSIFIER_TYPE_UML_DIAGRAM_REFERENCE, "Diagram Reference (interaction overview)", icon_clas_diagram );
        current = &((*this_).type_name_icon_list[idx]); idx++;
    ...
    (*this_).relationship_types_start = idx;
    {
        ...
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_CONTAINMENT, "Containment (deployment, package), no cycles", icon_rel_contain );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_deploy = gui_resources_get_type_rel_deploy( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_DEPLOY, "Deploy (deployment), no cycles", icon_rel_deploy );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_manifest = gui_resources_get_type_rel_manifest( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_MANIFEST, "Manifest (deployment), no cycles", icon_rel_manifest );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_COMMUNICATION_PATH, "Communication Path (component)", icon_simple_arrow );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_ASSOCIATION, "Association (class, use-case)", icon_simple_arrow );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_aggregate = gui_resources_get_type_rel_aggregate( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_AGGREGATION, "Aggregation (class), no cycles", icon_rel_aggregate );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_compose = gui_resources_get_type_rel_compose( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_COMPOSITION, "Composition (class), no cycles", icon_rel_compose );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_generalize = gui_resources_get_type_rel_generalize( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_GENERALIZATION, "Generalization (class), no cycles", icon_rel_generalize );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_realize = gui_resources_get_type_rel_realize( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_REALIZATION, "Realization (class), no cycles", icon_rel_realize );
        ...
        current = &((*this_).type_name_icon_list[idx]); idx++;
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_CONTROL_FLOW, "Control Flow (activity)", icon_simple_arrow );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_OBJECT_FLOW, "Object Flow (activity)", icon_simple_arrow );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_ASYNC_CALL, "Async. Call (sequence)", icon_simple_arrow );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_sync = gui_resources_get_type_rel_sync( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_SYNC_CALL, "Sync. Call (sequence)", icon_rel_sync );
        current = &((*this_).type_name_icon_list[idx]); idx++;
        const GdkPixbuf *icon_rel_return = gui_resources_get_type_rel_return( (*this_).resources );
        gui_type_resource_init_relationship( current, DATA_RELATIONSHIP_TYPE_UML_RETURN_CALL, "Return Call (sequence)", icon_rel_return );
    }
