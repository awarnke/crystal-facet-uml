...
void gui_sketch_area_private_refocus_and_reload_data ( gui_sketch_area_t *this_ )
{
    ...
    if ( GUI_TOOL_SEARCH != gui_sketch_request_get_tool_mode( &((*this_).request) ) )
    {
        if ( data_id_is_valid( &former_diagram_id )
            &&( DATA_ROW_VOID == gui_sketch_area_private_get_focused_diagram_id( this_ ) ))
        {
...
void gui_sketch_area_private_draw_subwidgets ( gui_sketch_area_t *this_, shape_int_rectangle_t area_bounds, cairo_t *cr )
{
    ...
    switch( selected_tool )
    {
        ...
        case GUI_TOOL_NAVIGATE:
        {
            ...
            /* draw link from highlighted nav_tree_node to diagram */
            const data_id_t highlighted = gui_marked_set_get_highlighted ( (*this_).marker );
            shape_int_rectangle_t nav_envelope_box;
            const u8_error_t nav_found
                = gui_sketch_nav_tree_get_node_envelope( &((*this_).nav_tree), &highlighted, &nav_envelope_box );
            const gui_sketch_card_t* card;
            const u8_error_t card_found = gui_sketch_area_private_get_card_of_id( this_, &highlighted, &card );
            if (( nav_found == U8_ERROR_NONE )&&( card_found == U8_ERROR_NONE ))
            {
                const shape_int_rectangle_t card_box = gui_sketch_card_get_bounds( card );
                gui_sketch_background_draw_appears_link( &((*this_).background), &nav_envelope_box, &card_box, cr );
            }
        }
        break;
        ...
        case GUI_TOOL_NAVIGATE:
        {
            ...
            else
            {
                ...
                if ( action_button_id != GUI_SKETCH_ACTION_NONE )
                {
                    ...
                    data_row_t new_diag_id = DATA_ROW_VOID;
        ...
        case GUI_TOOL_CREATE:
        {
            ...
            else
            {
                ...
                else /* clicked either into inner space of a classifier or of a diagram */
                {
                    ...
                    data_id_init( &dummy_classifier, DATA_TABLE_CLASSIFIER, DATA_ROW_VOID );
            ...
            if ( gui_sketch_drag_state_is_dragging ( &((*this_).drag_state) ) )
            {
                ...
                if ( data_id_is_valid( &dragged_classifier ) && data_id_is_valid( &destination_classifier ) && ( NULL != target_card ))
                {
                        ...
                        && ( DATA_TABLE_CLASSIFIER == data_id_get_table( &destination_classifier ) ) )
                    {
                        ...
                        data_row_t new_from_classifier_id;
                        data_row_t new_to_classifier_id;
                        ...
                        data_row_t new_to_feature_id;
                        {
                            ...
                            {
                                new_to_feature_id = DATA_ROW_VOID;
                            }
                        }
