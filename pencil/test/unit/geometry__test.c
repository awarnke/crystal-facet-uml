...
static test_case_result_t test_geometry_anchor( test_fixture_t *fix );
...
test_suite_t geometry__test_get_suite(void)
{
    ...
    test_suite_add_test_case( &result, "test_geometry_anchor", &test_geometry_anchor );
...
static test_case_result_t test_geometry_anchor_align( test_fixture_t *fix )
{
  ...
  geometry_rectangle_init( &my_bounds, 21.0, 2.0, 100.0, 0.1 );
  geometry_rectangle_t rect2 = geometry_anchor_align_biased_dim( &my_anchor, &my_dim2, &my_bounds );
  TEST_EXPECT_EQUAL_FLOAT( 26.0, geometry_rectangle_get_center_x( &rect2 ) );
  TEST_EXPECT_EQUAL_FLOAT( 2.05, geometry_rectangle_get_center_y( &rect2 ) );
